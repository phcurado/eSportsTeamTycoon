...
public class ConfirmContractScoutDialog extends GameScreenBox {
    public ConfirmContractScoutDialog(EsportsManager mainApp, Skin skin, String path, Screen root) {
    ...
    private void buttonsClick() {
        ...
        this.getActor("NoButton").addListener(new ClickListener() {
            public void clicked(InputEvent e, float x, float y) {
                Gdx.input.setInputProcessor(((ScoutReportScreen)root).getHireDialog().getStage());
                setVisibility(false);
            }
        });
    }
    ...
    private void transferLogic() {
        Team team = ((ScoutReportScreen)root).getHireDialog().player.playerTeam(mainApp.teamList);
        ...
        if(((ScoutReportScreen)root).getHireDialog().player.hasTeam(mainApp.teamList)) {
            //remove o player do time e contrata um novo
            ((ScoutReportScreen)root).getHireDialog().player.playerTeam(mainApp.teamList).getPlayers().remove(((ScoutReportScreen)root).getHireDialog().player);
            ...
            ((ScoutReportScreen) root).getHireDialog().player.setTeamId(mainApp.user.getTeam().getId());
            mainApp.user.getTeam().getPlayers().add(((ScoutReportScreen) root).getHireDialog().player);
            mainApp.user.getTeam().setBudget(mainApp.user.getTeam().getBudget() - ((ScoutReportScreen) root).getHireDialog().player.getCost(mainApp.contractList));
            mainApp.user.getTeam().organizeIdPlayers();
            usefulFunctions.changePlayerContract(mainApp.user.getTeam(), ((ScoutReportScreen) root).getHireDialog().player);
            team.setUpPlayers();
        }
        else if(!((ScoutReportScreen)root).getHireDialog().player.hasTeam(mainApp.teamList) ) {
            ...
            usefulFunctions.createNewContract(mainApp.user.getTeam(), ((ScoutReportScreen) root).getHireDialog().player, ((ScoutReportScreen) root).getHireDialog().player.getRecomendedCost(),
            ((ScoutReportScreen) root).getHireDialog().player.getRecomendedSalary());
        }
